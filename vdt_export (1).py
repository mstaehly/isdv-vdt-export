...
Nutzt die alte API (api.campai.com) fuer Kontakte/Mitglieder.
"""
...
def fetch_vdt_contacts():
    ...
    url = f"https://api.campai.com/contacts"
    headers = {
    ...
    while True:
        ...
        data = response.json()
        print(f"Response: {json.dumps(data)[:1000]}")

        # Alte API gibt entweder Liste oder Objekt mit items/data zurueck
...
def filter_new_members(contacts, date_from, date_to):
    ...
    new_members = []
    for c in contacts:
        # Verschiedene moegliche Felder fuer Eintrittsdatum
        entry_at = (
            c.get("membership", {}).get("entryAt") or
            c.get("member", {}).get("entryAt") or
            c.get("entryAt") or
            c.get("createdAt")
...
def build_excel(contacts):
    ...
    for c in contacts:
        ...
        # Vorname/Nachname aus verschiedenen moeglichen Strukturen
        personal = c.get("personal") or c.get("person") or {}
        vorname = (
            personal.get("personFirstName") or
            personal.get("firstName") or
            c.get("firstName") or ""
        )
        nachname = (
            personal.get("personLastName") or
            personal.get("lastName") or
            c.get("lastName") or ""
        )
        ...
        email = communication.get("email") or c.get("email") or ""
