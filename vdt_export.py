...
Sendet E-Mail ueber Microsoft Graph API (OAuth2).
"""
...
MS_CLIENT_ID      = "885986d0-de52-4505-9675-cbe1424674a6"
MS_TENANT_ID      = "a149971e-d956-4bd7-b024-a4e21ca238d3"
...
def fetch_vdt_contacts():
    """Alle Kontakte aus der alten Campai API abrufen."""
    ...
    while True:
        ...
        if not response.ok:
            print(f"Fehler {response.status_code}: {response.text[:500]}")
            response.raise_for_status()
...
def filter_new_members(contacts, date_from, date_to):
    ...
    for c in contacts:
        if not isinstance(c, dict):
            continue
        tags = c.get("mergeTags", {}).get("tags", "")
        if TAG_VDT not in str(tags):
            continue

        entry_at = (
            c.get("mergeTags", {}).get("enterDate") or
        ...
        if entry_at:
            try:
                # Format dd/mm/yyyy oder ISO
                if "/" in str(entry_at):
                    parts = str(entry_at).split("/")
                    entry_date = datetime(int(parts[2]), int(parts[1]), int(parts[0]), tzinfo=timezone.utc)
                else:
                    entry_date = datetime.fromisoformat(str(entry_at).replace("Z", "+00:00"))

                if date_from <= entry_date < date_to:
                    new_members.append(c)
            except Exception as e:
                print(f"Datum-Fehler: {entry_at} → {e}")
...
def build_excel(contacts):
    ...
    for c in contacts:
        mt = c.get("mergeTags", {})
        vorname = mt.get("personFirstName") or ""
        nachname = mt.get("personLastName") or ""
        email = mt.get("email") or c.get("communication", {}).get("email") or ""
        entry_at = mt.get("enterDate") or ""
...
def send_email_graph(token, zip_bytes, zip_filename, month_label, count):
    """E-Mail ueber Microsoft Graph API senden."""
    ...
    if not response.ok:
        print(f"Graph-Fehler: {response.status_code}: {response.text[:500]}")
        response.raise_for_status()
...
def main():
    ...
    send_email_graph(token, zip_bytes, zip_filename, month_label, len(new_members))
